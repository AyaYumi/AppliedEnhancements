...
仓库单元测试覆盖数量边界、AELIS 规划、循环执行、库存预留、公共 API 兼容、本地化及平台接入约定。使用 Gradle Wrapper 执行 `cleanTest build --no-daemon --console=plain` 可重新构建并运行测试。

默认 GameTest 任务不能复现历史独立验证环境的全部场景。客户端、服务端及可选模组联用仍需真实整合包验证；单元测试通过不等于完整运行兼容。
