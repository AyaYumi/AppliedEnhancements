...
- “分享到聊天”打开包含物品名称和 ID 的可编辑聊天草稿。

手持物品或容器时不会接管右键，AE2 原有的存入与容器填充行为保持不变。以 `[名称] namespace:path` 格式发送分享草稿后，安装了 Applied Enhancements 的客户端会把这段物品信息显示为带下划线的可点击链接。点击后由纯客户端命令打开玩家物品栏；JEI 启用注册 ID 搜索时按完整 ID 精确过滤，禁用时自动解析并搜索本地化物品名。隐藏搜索命令不会发送到服务器；未安装或尚未加载 JEI 时只显示本地提示。自定义数量请求在服务端重新校验当前菜单、同步序号、网络连接、能源、实时库存和玩家背包容量；客户端显示数量不能强制服务器多取物品。
